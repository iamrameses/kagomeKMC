...
def main():
    ...
    duration          = 0.0001     # Duration of the simulation in seconds
    frames_per_kelvin = 30      # Number of frames to sample per temperature change in Kelvin
    nwarmupsteps      = 25000  # Number of warm-up steps to take before the simulation starts
    progress_freq     = 250000  # Frequency of the progress output in KMC steps
    max_dt            = 10      # Maximum time step interval threshold in seconds
    save_file_path    = 'data/mols1200_100us_expTemp_noTle.pkl'  # Path to save the simulation data
    ...
    energy_params = {
        ...
        'lattice_params':{
            'lattice_constant':7.872,  # Lattice constant of the triangular lattice (nm)
    ...
    temperature_params = {
        ...
        'temp_final':8,         # Final temperature of the simulation (K)
        'method':'exponential'  # Cooling method can be 'linear', 'exponential', or 'inv_exponential'
    }

    # Molecule parameters for the simulation
    molecule_params = {
        'n_molecules':1200,  # Number of molecules to generate
        'threshold':18,      # Minimum distance (in lattice constants) between initial molecule positions
    }
